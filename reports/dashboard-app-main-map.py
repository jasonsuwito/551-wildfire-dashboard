        ...
        dbc.Row([
            ...
            dbc.Col([
                ...
                dbc.Row([
                    ...
                    dbc.Col(
                    	dbc.Card([
                            html.Div("Select Year Range:", style={"color": TEXT_COLOR, "fontSize": "18px", "fontWeight": "bold", "padding": "5px"}),
                            dcc.RangeSlider(
                                id="year-slider",
                                min=wildfires['YEAR'].min(),
                                max=wildfires['YEAR'].max(),
                                step=1,
                                marks={year: str(year) for year in range(wildfires['YEAR'].min(), wildfires['YEAR'].max() + 1)},
                                value=[wildfires['YEAR'].min(), wildfires['YEAR'].max()],  # Set initial range to cover all years
...
@app.callback(
    ...
    [Input("feature-dropdown", "value"),
     Input("year-slider", "value")]  # year-slider now returns a list of two values: start and end year
)
def update_map(selected_feature, selected_year_range):
    """Generate and update a wildfire map based on the selected feature and year range."""
    
    start_year, end_year = selected_year_range  # Unpack the start and end years
    
    # Filter data based on selected year range
    filtered_data = wildfires[(wildfires['YEAR'] >= start_year) & (wildfires['YEAR'] <= end_year)]
    ...
    if selected_feature == "SIZE_HA":
        ...
        data_agg = filtered_data.groupby(["LATITUDE", "LONGITUDE"], as_index=False).agg({"SIZE_HA": "mean"})
        ...
        if selected_feature == "MONTH":
            filtered_data['MONTH'] = filtered_data['MONTH'].astype(str)  # Convert to string for categorical
        fig = px.scatter_mapbox(
            filtered_data,
            lat="LATITUDE",
            lon="LONGITUDE",
        ...
        data_agg = filtered_data.groupby(["LATITUDE", "LONGITUDE"], as_index=False).agg({"FID": "count"})
        fig = px.scatter_mapbox(
            data_agg,
            lat="LATITUDE",
            lon="LONGITUDE",
